...
static void init_seq_buffer_with_trailing_nines_impl(
...
static uint16_t call_incr_cb(char *buf, uint16_t first_digit_offset,
                             uint16_t num_digits) {
  char *first_digit = buf + first_digit_offset;
  uint16_t new_num_digits = num_digits;

  incr_impl(&first_digit, &new_num_digits);
  return (uint16_t)(((uint16_t)(first_digit - buf) << 8) |
...
int main(void) {
  ...
  init_seq_buffer_with_trailing_nines_impl(
      buf, num_zeroes, num_digits, nine_position, digit0, digit1, digit2,
      digit3, digit4, digit5, digit6);
  first_digit_offset = num_zeroes;

  for (i = 0; i < SEQ_BUF_SLOTS; ++i)
    old_buf[i] = buf[i];
  old_num_digits = num_digits;

  call_result = call_incr_cb(buf, first_digit_offset, num_digits);
  first_digit_offset = (uint16_t)(call_result >> 8);
  num_digits = (uint16_t)(call_result & 0x00ff);

  if (first_digit_offset > 8)
    return 0;
  if (num_digits == 0 || num_digits > 9)
    return 0;
  if (first_digit_offset + num_digits > 9)
    return 0;

  first_digit = buf + first_digit_offset;
  klee_assert(
      is_incremented_with_carry(old_buf + num_zeroes, old_num_digits,
