...
static char *decimal_absval_add_one_positive_impl(char *buf) {
  char *absnum = buf + 1;
...
typedef struct DecimalAbsvalAddOnePositiveResult {
  uint16_t byte0;
  uint16_t byte1;
  uint16_t byte2;
  uint16_t byte3;
  ...
  uint16_t byte9;
  uint16_t ret;
} DecimalAbsvalAddOnePositiveResult;

DecimalAbsvalAddOnePositiveResult decimal_absval_add_one_cb(
  ...
  char *result;
  DecimalAbsvalAddOnePositiveResult out;

  load_buf(buf, in0, in1, in2, in3, in4, in5, in6, in7, in8, in9);
  result = decimal_absval_add_one_positive_impl(buf);
