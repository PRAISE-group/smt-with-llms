...
clang -std=gnu2x -Wall -Wextra -pedantic -O0 \
    "$script_dir/decimal_length_bucket.c" \
    -o "$script_dir/decimal_length_bucket"

"$script_dir/decimal_length_bucket"

clang -std=gnu2x -I "$klee_include_dir" -emit-llvm -c -g -O0 -Xclang -disable-O0-optnone \
    "$script_dir/decimal_length_bucket_klee.c" \
    -o "$script_dir/decimal_length_bucket_klee.bc"

rm -rf "$script_dir/klee-out"
"$klee_bin" --write-smt2s --write-kqueries --write-paths \
    --output-dir="$script_dir/klee-out" \
    "$script_dir/decimal_length_bucket_klee.bc"
