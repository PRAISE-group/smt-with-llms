...
uint32_t fg_array_search_4_cb (uint32_t y1, uint32_t y2, uint32_t y3, uint32_t y4, uint32_t k1) {
...
uint32_t fg_array_sum_2_15_cb(uint32_t y1, uint32_t y2) {
  uint32_t _let_1 = y1 + y2;
  if (_let_1 >= 16 && (y1 != -y2)) return _let_1;
  else return 0;
}

uint32_t findSum4_cb (uint32_t y1, uint32_t y2, uint32_t y3, uint32_t y4) {
...
uint32_t f_jmbl_fg_VC22_a_cb(uint32_t x1, uint32_t x2) {
  if (x1 == x2) return 0;
  else return 1;
}

uint32_t InV1_cb (uint32_t v, uint32_t V_min, uint32_t V_mux_) {
  if (v >= V_min)
    if (v <= V_mux_)
      return 1;
...
uint32_t InV2_cb (uint32_t v1, uint32_t v2, uint32_t V_min, uint32_t V_mux_) {
  if (v1 >= V_min)
    if (v1 <= V_mux_)
      if (v2 >= V_min)
	if (v2 <= V_mux_)
...
uint32_t ex_cb(uint32_t x, uint32_t y) {
  if ((x < 5) && ((2 * x) != ((-3 * y) - 16 ))) {
    return 3 * x + 1;
  }
  else
...
uint32_t LinExpr1_eq1ex_cb(uint32_t x, uint32_t y, uint32_t z) {
  if ((y + 1) == x) return z;
  else return x;
}
