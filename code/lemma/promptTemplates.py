...
LEMMA_REFINEMENT_TEMPLATE = """
    I appreciate the response to the tasks so far. However, some of the <ARTIFACT>s generated by you are not valid.  
   
    - INVALID_LEMMAS: 
    The following <ARTIFACT> that you generated are not correct.
    ...
    1) Do not generate <ARTIFACT> which are like the ones in INVALID_LEMMAS.
    2) Be careful about the syntax correctness and adhere to <FORMAT>.
    3) Do not repeat the <ARTIFACT> that are already present in ALL_LEMMAS.

    Please generate new lemmas between LEMMA_START and LEMMA_END blocks that adhere to the rules stated above.
"""
